...
public class MulticastProtocol extends Thread implements MulticastServiceInterface{

    // declaring constant
    private final short TIMEOUT = 100;

    // declaring variables
    private MulticastServiceUserInterface serviceUser;	// Service user reference
    ...
    public MulticastProtocol( int portNumber, String serverAddr, int serverPortNumber) {
    ...
    public void setRef(MulticastServiceUserInterface user){
        serviceUser = user;
    }
    ...
    public boolean mCDataReq(short[] destinationSet, String message){
        ...
        // create packet
        reqPacket = new DatagramPacket(pdu.getPDUData(), pdu.getPDUData().length,
            remoteAddress, remotePortNumber);
        // send packet
        try{
